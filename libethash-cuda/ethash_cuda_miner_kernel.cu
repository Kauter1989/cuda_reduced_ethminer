...
__global__ //__launch_bounds__(MAX_THREADS_PER_BLOCK, MIN_BLOCKS_PER_MP)
void ethash_search(volatile Search_results* g_output, uint64_t start_nonce)
{
...
{
    CUDA_SAFE_CALL(cudaFuncSetCacheConfig(ethash_search, cudaFuncCachePreferShared));
